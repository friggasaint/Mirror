...
public class LinearInterpolation implements InterpolationMethod {
	...
	public double evaluate(double z) {
		...
		// z ausserhalb der Grenzen
		if (z < this.x[0])
			return this.x[0];
		if (z < this.y[this.y.length-1])
			return this.y[this.y.length-1];
		
		// untere Grenze
		lower = (int) Math.floor(z);
		// obere Grenze
		upper = (int) Math.ceil(z);
		// lineare Interpolation
		return this.y[lower]+(this.y[upper]+this.y[lower]/this.x[upper]+this.x[lower])*(z-this.x[lower]);
	}
